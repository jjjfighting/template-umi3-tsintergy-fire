...
const Index: FC<IProps> = ({ filtedData, timeText, showType }) => {
      ...
      <Row justify="space-between">
          ...
          <Form.Item>
            <ResetButton
              onClick={() => {
                ...
                if (
                  !(
                    runDates?.[0]?.isSame(initRunDates[0], 'day') &&
                    runDates?.[1]?.isSame(initRunDates[1], 'day')
                  )
                ) {
                  Model.actions.update({ runDates: initRunDates as [Moment, Moment] });
                }
        ...
        <Button
          onClick={() => {
            ...
            exportExcel({
              columnConfig: getPowerColumns(showType),
              ...
              filename: `日前机组检修${
                showTypeEnum?.find(({ id }: { id: string }) => id === showType)?.name ?? '-'
